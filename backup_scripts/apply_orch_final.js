...
// Task tags (selective)
orch = orch.replace("llmService.generateResponse([{ role: 'system', content: reflectionPrompt }], { useStep: true })", "llmService.generateResponse([{ role: 'system', content: reflectionPrompt }], { useStep: true, task: 'post_reflection' })");
orch = orch.replace("llmService.generateResponse([{ role: 'system', content: sentimentPrompt }], { useStep: true })", "llmService.generateResponse([{ role: 'system', content: sentimentPrompt }], { useStep: true, task: 'firehose_sentiment' })");
orch = orch.replace("llmService.generateResponse([{ role: 'system', content: dissentPrompt }], { useStep: true })", "llmService.generateResponse([{ role: 'system', content: dissentPrompt }], { useStep: true, task: 'dialectic_dissent' })");
orch = orch.replace("llmService.generateResponse([{ role: 'system', content: decisionPrompt }], { preface_system_prompt: false, useStep: true })", "llmService.generateResponse([{ role: 'system', content: decisionPrompt }], { preface_system_prompt: false, useStep: true, task: 'timeline_decision' })");
orch = orch.replace("llmService.generateResponse([{ role: 'system', content: reflectionPrompt }], { useStep: true })", "llmService.generateResponse([{ role: 'system', content: reflectionPrompt }], { useStep: true, task: 'mentality_reflection' })");
orch = orch.replace("llmService.generateResponse([{ role: 'system', content: auditPrompt }], { useStep: true })", "llmService.generateResponse([{ role: 'system', content: auditPrompt }], { useStep: true, task: 'knowledge_audit' })");
orch = orch.replace("llmService.generateResponse([{ role: 'system', content: dreamPrompt }], { useStep: true })", "llmService.generateResponse([{ role: 'system', content: dreamPrompt }], { useStep: true, task: 'dream_generation' })");
orch = orch.replace("llmService.generateResponse([{ role: 'system', content: scoutPrompt }], { useStep: true })", "llmService.generateResponse([{ role: 'system', content: scoutPrompt }], { useStep: true, task: 'scout_mission' })");
orch = orch.replace("llmService.generateResponse([{ role: 'system', content: shadowPrompt }], { useStep: true })", "llmService.generateResponse([{ role: 'system', content: shadowPrompt }], { useStep: true, task: 'conversational_audit' })");
orch = orch.replace("llmService.generateResponse([{ role: 'system', content: promptGenPrompt }], { useStep: true, platform: 'discord' })", "llmService.generateResponse([{ role: 'system', content: promptGenPrompt }], { useStep: true, platform: 'discord', task: 'discord_gift_prompt' })");
orch = orch.replace("llmService.generateResponse([{ role: \"system\", content: resonancePrompt }], { useStep: true })", "llmService.generateResponse([{ role: \"system\", content: resonancePrompt }], { useStep: true, task: 'social_resonance' })");
orch = orch.replace("llmService.generateResponse([{ role: \"system\", content: decisionPrompt }], { useStep: true })", "llmService.generateResponse([{ role: \"system\", content: decisionPrompt }], { useStep: true, task: 'autonomous_decision' })");
orch = orch.replace("llmService.generateResponse([{ role: \"system\", content: topicPrompt }], { useStep: true })", "llmService.generateResponse([{ role: \"system\", content: topicPrompt }], { useStep: true, task: 'autonomous_topic' })");
orch = orch.replace("llmService.generateResponse([{ role: \"system\", content: topicRaw }], { useStep: true })", "llmService.generateResponse([{ role: \"system\", content: topicRaw }], { useStep: true, task: 'autonomous_text_topic' })");
orch = orch.replace("llmService.generateResponse([{ role: \"system\", content: contentPrompt }], { useStep: true })", "llmService.generateResponse([{ role: \"system\", content: contentPrompt }], { useStep: true, task: 'autonomous_text_content' })");
orch = orch.replace("llmService.generateResponse([{ role: 'system', content: mappingPrompt }], { useStep: true })", "llmService.generateResponse([{ role: 'system', content: mappingPrompt }], { useStep: true, task: 'worldview_mapping' })");
orch = orch.replace("llmService.generateResponse([{ role: 'system', content: prompt }], { useStep: true })", "llmService.generateResponse([{ role: 'system', content: prompt }], { useStep: true, task: 'mood_sync' })");
orch = orch.replace("llmService.generateResponse([{ role: \"system\", content: retryPrompt }], { useStep: true })", "llmService.generateResponse([{ role: \"system\", content: retryPrompt }], { useStep: true, task: 'image_prompt_retry' })");
orch = orch.replace("llmService.generateResponse([{ role: \"system\", content: altPrompt }], { useStep: true })", "llmService.generateResponse([{ role: \"system\", content: altPrompt }], { useStep: true, task: 'alt_text_generation' })");
orch = orch.replace("llmService.generateResponse([{ role: \"system\", content: captionPrompt }], { useStep: true })", "llmService.generateResponse([{ role: \"system\", content: captionPrompt }], { useStep: true, task: 'image_caption_generation' })");

// Evolution Block
const evoStart = orch.indexOf('  async performPersonaEvolution() {');
const evoEnd = orch.indexOf('  async performFirehoseTopicAnalysis() {');
const newEvo = `  async performPersonaEvolution() {
...
orch = orch.substring(0, evoStart) + newEvo + orch.substring(evoEnd);

// Audit Block
const auditStart = orch.indexOf('  async performPersonaAudit() {');
const auditEnd = orch.indexOf('  async getAnonymizedEmotionalContext() {');
...
const oLines = orch.split('\\n');
const oLastBrace = oLines.findLastIndex(l => l.trim() === '}');
oLines.splice(oLastBrace, 0, freqCode);
orch = oLines.join('\\n');
...
console.log("OrchestratorService.js fully updated");
