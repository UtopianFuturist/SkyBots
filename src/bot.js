...
5. **Vary Your Vocabulary (ANTI-SLOP):**
...
export class Bot {
  ...
  async init() {
    ...
    discordService.init().then(() => {
        ...
        console.error('[Bot] CRITICAL: DiscordService.init() failed to even start:', err);
    });
  ...
  async checkMaintenanceTasks() {
    ...
    if (discordService.isEnabled && dataStore.getDiscordAdminAvailability()) {
        ...
        if (admin) {
            ...
            // History recovery for heartbeat if empty
            if (history.length === 0) {
                try {
                    const dmChannel = await admin.createDM();
                    history = await discordService.fetchHistory(dmChannel, 20);
                } catch (e) {
                    console.warn(`[Bot] Failed to recover history for heartbeat:`, e);
                }
            }
            ...
            if (quietMins >= 20) {
                ...
                // Repetition check against last 15 assistant messages in history
                ...
                if (message && message.toUpperCase() !== 'NONE' && !isRepetitive) {
                    ...
                    console.log(`[Bot] Discord heartbeat suppressed: Generated message was too similar to recent history (similarity check: 0.4 threshold).`);
                }
            } else {
    ...
    try {
      ...
      if (postType === 'image' && config.IMAGE_SUBJECTS) {
        ...
        topicPrompt = `
          ...
          INSTRUCTION: Review the "Preferred Topics" and the current network context.
          You should prioritize selecting a topic that aligns with your configured POST_TOPICS if available and relevant, but you can also choose to post about recent interesting interactions or network buzz.
      ...
      if (musingRaw) {
        ...
        if (titleMatch && contentMatch) {
          ...
          if (checkSimilarity(content, recentMoltbookPosts, 0.35)) {
            console.warn(`[Moltbook] Generated musing is too similar to recent posts (threshold: 0.35). Skipping.`);
            return;
          }
