...
class DataStore {
  ...
  async addSessionLesson(text) {
    this.db.data.session_lessons.push({ text, timestamp: Date.now() });
  ...
  async addGrowthEntry(entry) {
    this.db.data.growth_log.push({ ...entry, timestamp: Date.now() });
    await this.write();
  }
  ...
  async addRefusal(reason) {
    this.db.data.refusal_logs.push({ reason, timestamp: Date.now() });
    if (this.db.data.refusal_logs.length > 50) this.db.data.refusal_logs.shift();
    await this.write();
  }
  getRefusalCounts() { return { global: this.db.data.refusal_logs.length, discord: 0, bluesky: 0 }; }
  ...
  getSelfModel() { return this.db.data.self_model; }
  searchInternalLogs(query) { return this.db.data.internal_logs.filter(l => JSON.stringify(l).includes(query)); }
  ...
  async updateCurrentGoal(goal) {
    this.db.data.current_goal = { ...goal, timestamp: Date.now() };
    await this.write();
    if (memoryService.isEnabled()) await memoryService.createMemoryEntry('goal', `[GOAL] ${goal.goal}: ${goal.description}`);
  }
