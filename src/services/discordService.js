...
class DiscordService {
    ...
    async loginLoop() {
        ...
        const RETRY_INTERVAL = 45 * 60 * 1000; // 45 minutes
        let attempts = 0;
        ...
        while (true) {
            ...
            try {
                ...
                console.error(`[DiscordService] [Attempt ${attempts}] Login failed:`, err.message);
                
                if (err.message.includes('TOKEN_INVALID')) {
                    console.error('[DiscordService] FATAL: Invalid token. Stopping login loop.');
                    this.isInitializing = false;
                    return;
                }

                console.log(`[DiscordService] Next login attempt in 45 minutes (${new Date(Date.now() + RETRY_INTERVAL).toLocaleTimeString()})...`);
    ...
    async handleMessage(message) {
        ...
        if (isDM || isMentioned || isReplyToMe) {
            ...
            await dataStore.saveDiscordInteraction(this.getNormalizedChannelId(message), 'user', text);
    ...
    async respond(message) {
        ...
        try {
            ...
            if (finalResponse) {
                ...
                await dataStore.saveDiscordInteraction(normChannelId, 'assistant', finalResponse);
            } else {
