...
class DiscordService {
    ...
    async fetchHistory(channel, limit = 20) {
        if (!channel) return [];
        try {
            console.log(`[DiscordService] Fetching history from Discord API for channel ${channel.id}...`);
            ...
            console.log(`[DiscordService] Successfully fetched ${history.length} messages.`);
            return history;
        } catch (err) {
    ...
    async respond(message) {
        ...
        if (history.length === 0) {
            history = await this.fetchHistory(message.channel, 20);
        }
