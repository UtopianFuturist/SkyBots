...
IMPORTANT: Respond directly with the requested information. DO NOT include any reasoning blocks, <think> tags, or internal monologue in your response.`;
  ...
  async isImageCompliant(imageSource) {
    ...
    try {
      ...
      // Default to compliant on error to avoid blocking the bot, or should we default to non-compliant?
      // Given the user wants to avoid humans, maybe non-compliant is safer, but "compliant" is better for availability.
      // Let's assume compliant but log the error.
      return { compliant: true, reason: null };
    } finally {
      clearTimeout(timeout);
    }
